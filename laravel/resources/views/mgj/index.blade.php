    ...
    <div class="container">
        <div class="row">
            <div class="col-2">
                <!-- TOMBOL TAMBAH DATA -->
                <div class="pb-3">
                    <a href='{{ url('mgj/create') }}' class="btn btn-md btn-primary">
            ...
            <div class="col-4">
                <!-- FORM PENCARIAN -->
                <div class="pb-3">
                    <form class="d-flex" action="{{ url('mgj') }}" method="get">
            ...
            <tr>
                ...
                <td>
                    {{-- <button type="button" class="btn btn-primary btn-sm viewdetails" data-bs-toggle="modal"
                        data-bs-target="#details" data-id="{{ '/mgj/'.$item->mgj }}">
                        Detail
                    </button> --}}
                    <a href='{{ url('mgj/'.trim($item->mgj).'/edit') }}' class="btn btn-warning btn-sm">Edit</a>
                    <form onsubmit="return confirm('Yakin akan menghapus data {{ trim($item->mgj) }}?')" class="d-inline"
                        action="{{ url('mgj/'. $item->mgj) }}" method="post">
                        @csrf
                        @method('DELETE')
